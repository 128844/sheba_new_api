...
class AttendanceTransformer extends TransformerAbstract
{
     ...
     */
    public function __construct(TimeFrame $time_frame, $business_holiday, $weekend_settings, $business_member_leave, $business_office_repo, $dayWiseShifts)
    {
    ...
    public function transform($attendances)
    {
        ...
        foreach ($period as $date) {
            ...
            $is_in_shift = $shift && $shift->isInShift() ? 1 : 0;
            $is_unassigned = $shift && $shift->isUnassigned() ? 1 : 0;
            $breakdown_data = [];

            $weekend_day = !$is_in_shift ? $check_weekend->getWeekendDays($date, $this->businessWeekendSettings) : [];
            $is_weekend_or_holiday = !$is_in_shift ? $this->isWeekendHoliday($date, $weekend_day, $dates_of_holidays_formatted) : null;
            $is_on_leave = $this->isLeave($date, $leaves);

            $breakdown_data['weekend_or_holiday_tag'] = null;
            $breakdown_data['is_holiday'] = 0;
            $weekend_holiday = null;
            if ($is_weekend_or_holiday || $is_on_leave || $is_unassigned) {
                if ($is_unassigned) $weekend_holiday = "It is a Holiday!";
                else if (!$is_in_shift) $weekend_holiday = $this->isWeekendHolidayLeaveTag($date, $leaves_date_with_half_and_full_day, $dates_of_holidays_formatted);;
                $breakdown_data['weekend_or_holiday_tag'] = $weekend_holiday;
                $breakdown_data['is_holiday'] = $weekend_holiday == null ? 1 : 0;
            }
            if ($is_weekend_or_holiday || $is_unassigned) {
            ...
            if ($this->isAbsent($attendance, ($is_weekend_or_holiday || $this->isFullDayLeave($date, $leaves_date_with_half_and_full_day) || $is_unassigned), $date)) {
