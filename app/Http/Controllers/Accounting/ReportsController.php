...
class ReportsController extends Controller
{
    ...
    public function getAccountingReport(Request $request, $reportType): JsonResponse
    {
        ...
        $start_date = $request->start_date ? $request->start_date . ' 0:00:00' : Carbon::now()->format('Y-m-d') . ' 0:00:00';
        $end_date = $request->end_date ? $request->end_date . ' 23:59:59' : Carbon::now()->format('Y-m-d') . ' 23:59:59';
    ...
    public function getTransactionList(Request $request): JsonResponse
    {
        ...
        $data = $this->accountingReportRepository
            ->setLimit($request->limit)
            ->setOffset($request->offset)
            ->setStartDate($request->start_date)
            ->setEndDate($request->end_date)
            ->setTransactionType($request->transaction_type)
            ->setReconcile($request->reconcile)
            ->setGateway($request->gateway)->setQ($request->q)
